...
/*--------------------------------------------------------------------*/
double     adaptive_threshold_update(AdaptiveThreshold* self, double     x, double* filtered_x)
{
  double     next   = 0;
  double     result = 0;
  ...
  if(filtered_x != NULL)
    {
      double std_dev = moving_average_std_dev(self->avg);
      if(std_dev < self->min)
        std_dev = self->min;
        *filtered_x = (self->filtered_x - moving_average_mean(self->avg)) / std_dev;
    }
